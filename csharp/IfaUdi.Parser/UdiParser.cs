...
namespace IfaUdi.Parser
{
    public static class UdiParser
    {
        ...
        private static UdiDi ParsePpn(string value)
        {
            ...
            if (!pzn.All(c => c >= '0' && c <= '9'))
            {
                throw new IfaUdiFormatException($"PZN '{pzn}' must be 8 digits.");
            }

            string expectedCheck = CheckDigits.Mod97(value.Substring(0, 10));
            ...
            char expectedPznCheck = CheckDigits.Mod11Pzn(pzn.Substring(0, 7));
            ...
            foreach ((string di, string value) in fields)
            {
                switch (di)
                {
                    ...
                    case "S":
                        ...
                        break;
                    case "Q":
                        if (value.Length < 1 || value.Length > 8 || !value.All(c => c >= '0' && c <= '9'))
                        {
                    ...
                    case "8P":
                        ...
                        if (additionalGtins is null)
                        {
                            additionalGtins = new List<string>();
                        }
        ...
        private static string ParseDate(string value, bool expectFourDigitYear)
        {
            ...
            if (value.Length != expectedLength || !value.All(c => c >= '0' && c <= '9'))
            {
